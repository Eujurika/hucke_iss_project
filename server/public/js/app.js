async function drawApp() {
  ...
  */
  ...
  const scale = 350; // Fester Skalierungsfaktor
  const cx = 400;
  const cy = 225;
  ...
  function getLatitude(angle) {
    ...
    /*  const latRotateScale = d3
      .scaleLinear()
      //.domain([0, 180, 181, 360])
      //.range([0, -180, 179, 0]);
      .domain([0, 180])
      .range([0, -180]);
    const lat = latRotateScale(angle);

    const latRotateInversScale = d3
      .scaleLinear()
      //.domain([0.0, -180.0, 179.0, 1.0])
      //.range([0.0, 180.0, 181.0, 359.0]);
      .domain([0, -180])
      .range([0, 180]);
    const angleOflat = latRotateInversScale(lat);
    //console.log(`${angle}/${lat}/${latRotateScale.invert(lat)}`); */
    //console.log(`${angle}/${lat}/${angleOflat}`);
